...
public class WebUI {
    ...
    public static List<String> getListElementsTextSmart(By by) {
        smartWait();
    ...
    public static void showLog(){
        LogUtils.info("Passed verify at line %s: in method %s - file %s".formatted(Thread.currentThread().getStackTrace()[3].getLineNumber(),
    ...
    @Step("Verify Contains Ignore Case: {0} ---AND--- {1}")
    public static boolean verifyContainsIgnoreCase(String value1, String value2) {
        boolean result = value1.toLowerCase().contains(value2.toLowerCase());
        if (result == true) {
            LogUtils.info("Verify Equals: " + value1 + " CONTAINS " + value2);
            showLog();
            if (ExtentTestManager.getExtentTest() != null) {
    ...
    @Step("Verify Contains Ignore Case: {0} ---AND--- {1}")
    public static boolean verifyContainsIgnoreCase(String value1, String value2, String message) {
        boolean result = value1.toLowerCase().contains(value2.toLowerCase());
        if (result == true) {
            LogUtils.info("Verify Equals: " + value1 + " CONTAINS " + value2);
            showLog();
    ...
    public static void scrollToEndOfPage() {
        ...
        LogUtils.info("Scroll to end of page" );
    }
     ...
     */
    @Step("Click on the element {0}")
    public static void clickElementWithExpectedCondition(By by) {
        ...
        for(int attempts = 1; attempts <= 3; attempts++) {
            try {
                ...
                LogUtils.error("Failed to click element: " + by.toString());
            }
        }
        if(result == false) {
            Assert.fail("Failed to click element after 3 retries: " + by.toString());
        }
        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Clicked on the element " + by.toString());
        }
     ...
     */
    @Step("Click on the element {0}")
    public static void smartClickElement(By by) {
        try {
            waitForElementVisible(by).click();
            LogUtils.info("Clicked on the element " + by.toString());
        }
    ...
    public static String getTextElement(By by) {
        ...
        AllureManager.saveTextLog("==> The Text is: " + text);
        LogUtils.info("Get text from " + by + " is: " + text);
        return text.trim();
    }
     ...
     */
    @Step("Get text of element {0}")
    public static String getTextElementSmart(By by) {
        smartWait();
        String text = "";
        AllureManager.saveTextLog("Get text of element " + by.toString());
        try {
            text = waitForElementVisible(by).getText();
        } catch (StaleElementReferenceException e){
            text = waitForElementVisible(by).getText();
        }
     ...
     */
    @Step("Get attribute {1} of element {0}")
    public static String getAttributePresentElement(By by, String attributeName) {
        smartWait();
        return waitForElementPresent(by).getAttribute(attributeName);
    }
    ...
    //Handle Table
    /**
     * Lấy giá trị dựa vào hàng và cột của table
     *
     * @param row vị trí hàng
     * @param column vị trí cột
     * @return value của hàng cột
     */
    @Step("Get data on the Table by Row and Column.")
    public static String getValueOnTableByRowColumn(int row, int column) {
        smartWait();
        sleep(1);
        List<WebElement> totalRows = DriverManager.getDriver().findElements(By.xpath("//tbody/tr"));
        if(totalRows.size() < 1){
            LogUtils.info("Not found any value in table");
            return "";
        } else {
            String value = getTextElement(By.xpath("//tbody/tr[%s]/td[%s]".formatted(row, column)));
            LogUtils.info("Value from table at row %s column %s is %s".formatted(row, column, value));
            return value;
        }
     ...
     * @return một đối tượng WebElement đã sẵn sàng để thao tác
     */
    public static WebElement waitForElementVisibleWithoutAssert(By by, long timeOut) {
        smartWait();
